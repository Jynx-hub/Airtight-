...
1. **Statute-blind retrieval — CORRECTNESS, gates the Track-1 claim.** `agent/memory.py` `retrieve()` ranks `(class, keyword_overlap, id)`; **statute type is not a signal, and `LoopholeRecord` has no statute field.** Two live ablation runs disagree (distilled-FWD corpus warmed 5/6; office-action corpus ran backwards 4/10→1/10) — exactly what a statute-blind ranker produces. **No delta number is trustworthy until this is fixed and both corpora re-run.** Owner: agent lane (Anudeep) + shape change touches the cross-lane `LoopholeRecord`.
