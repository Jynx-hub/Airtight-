...
|---|---|---|---|---|
| **1 · Data** (Sreesanth) | **E1–E4** | E5 | — | **50 real G06N patents + 76 real office-action defects on disk**, loading natively into P4's harness. Pulled by `data/pull_uspto.py --groundtruth`; the 4,500-line `src/` pipeline stays dead and should be quarantined. |
...
- [x] **E2** `feat/data-groundtruth` — Pull the **PTAB decisions dataset** + office-action rejections. For each patent in our classes: *which claims died, and why.* This is the scoring key for everything.
  ...
  **Route changed, deliberately:** office actions, *not* the PTAB decisions dataset — an OA names the claim, the statute and the prior art in a single fixed grammar, which PTAB trial metadata does not. `--ptab` still emits raw records and `distill_loopholes.py` still does not exist; that leg is now **redundant rather than blocking**.
  ⚠️ **`src/extractors/oa_extractor.py` is not on the path** — a correction to the earlier audit, which recommended salvaging it. Its 23/23 tests pass against a clean synthetic fixture, but its rationale windows centre on marker words, so on real office actions they land on the obviousness *reasoning* ("It would have been obvious to one of ordinary skill…") and never contain the claim list. It produced claim shapes like `"(Canceled) Canceled 3"` and *"Determining the scope and contents of the prior art"* (Graham v. Deere boilerplate, not a claim). The file is untouched and still passes its tests; it simply isn't used.
...
- [x] **E4** `feat/data-loaders` — Simple loaders that hand all of the above to Person 4's code in the agreed shapes. A folder of clean JSON beats a database — simplest thing that works.
  **Done — by output layout, not by new loader code.** The lane's "Done when" is that Person 4 can load with one call each; the puller now writes the exact directories `agent/eval/harness.py` already reads, so `load_disclosures(Path('data/real/disclosures'))` → 50 records and `load_checklist(Path('data/real/checklists'), id)` → that disclosure's held-out set, both with zero new code. Writing loaders nobody imports was the original failure mode; matching the consumer's layout is the simpler thing that works.
  ⚠️ **`src/loaders.py` is still 318 lines of dead, broken code** — nothing imports it, and all 5 functions still fail against the repo's own files (`load_fixtures` globs `disc_*.json` against `disc-0001.json`; `load_checklists` reads a directory that doesn't hold them; the other three `FileNotFoundError`). **Delete it** — leaving a plausible-looking loader that silently returns `[]` next to a working path is a trap for whoever reads this lane next.
...
**→ Met, except the poisoned PDF.** Corpus, checklists and disclosures all load through the harness's own calls, and the no-overlap proof is green on real data (0 collisions, 0 Jaccard flags). The poison is still a `.txt` with no hiding layer (E5). **Remaining work in this lane, in order:** (1) run `src/poison_builder.py` to produce the actual PDF and close E5; (2) quarantine the two fabricators and delete `src/loaders.py`; (3) broaden the corpus to G06F/H04L if there's time.
