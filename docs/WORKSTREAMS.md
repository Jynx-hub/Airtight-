...
| **4 · Agent** (Anudeep) | **G0** | G1, G2, **G3**, G4 | G5 | Most-built lane. **G3 has now touched real infrastructure** — the ablation ran live against Nemotron on real USPTO data 2026-07-18 and showed the effect; the judged 10-disclosure run stalled on a missing call timeout and is not yet on disk. The rest is still stub/simulation. |

**The three things that decide whether this demos:**

1. **G3 has now run live — and the effect is real, but the judged run is not yet on disk.** E1+E2 landed and the harness was wired to the real pull (`--layout pooled`), so on **2026-07-18 the ablation ran against live Nemotron on real USPTO data for the first time.** A 2-disclosure pilot showed the Track-1 signal on all three axes at once — on `uspto-19206781` the warmed arm caught **1/1 vs 0/1**, drafted **21s faster** (28.5s vs 49.6s) and left **half the defects** (5 vs 10), retrieving 5 records all from *other* applications with the overlap guard clean. The second disclosure moved 0/1 on both arms — no signal, on a 1-record checklist. **The judged `--n 10` run then stalled at 5 of 10 disclosures and had to be killed** (see the timeout bug below), so `results.json` for the headline run still does not exist. Re-running it is the single highest-leverage action on the board; the harness change needed first is a per-call timeout.
