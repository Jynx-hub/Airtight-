"""
data/poison_builder.py  [E5]
============================
Builds the booby-trapped prior-art PDF for the security demo.
...
The read side that catches both vectors is `agent/ingest.py`: on a `.pdf` it
runs pdfplumber's `extract_text()` over every page and joins the `pdf.metadata`
values, then hands the combined string to the HiddenLayer guardrail bus. Either
vector independently surfaces the trigger.

Run it:
    python -m data.poison_builder            # writes data/fixtures/poisoned_prior_art.pdf

Dependencies (the `poison` extra): reportlab (write) + pdfplumber (read).
    pip install -e ".[poison]"
"""
...
# Output lives next to the plain-text fixture so both feed the guardrail tests.
...
class PoisonBuilder:
    ...
    def build(self) -> Path:
        ...
        out_path = self.output_dir / "poisoned_prior_art.pdf"
...
if __name__ == "__main__":
    path = build_poison_pdf()
    print(f"[airtight:poison] wrote {path} ({path.stat().st_size:,} bytes)")
    print("[airtight:poison] scan it: python -m agent.ingest "
          f"{path} --fake-detect")
