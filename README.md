...
# expect: 156 passed — no network, no .env, no GPU
```
...
`importorskip` pdfplumber *by design*, so a `.[dev]`-only clone reports a green **171 passed,
34 skipped** — green, but with all 32 surface tests and both poison-PDF tests silently
not run (`.[dev,web]` without `poison` is **203 passed, 2 skipped**). **205 passed** (no
