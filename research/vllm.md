...
1. **Rent a GPU** — use **Brev** (we get $100 Brev credits per team member from the NemoClaw + Nemotron bounties). SSH in.
2. `pip install vllm`, then `vllm serve <model>` — it downloads weights and starts an OpenAI-compatible server on a port.
3. Point `inference.local` at that server's URL. The route is **operator-pinned**, so the agent inside the OpenShell sandbox cannot repoint it — that's the containment property. vLLM is the engine; `inference.local` is the locked pipe to it.
...
**Key caveat — VRAM:** even at ~12B active, the 120B MoE must hold all params in memory. On event/Brev hardware that can be tight. Serve **Nano on vLLM as the guaranteed path**; bring up Super only if the GPU allows.
...
- Exact `vllm serve` flags for the chosen Nemotron variant (check the vLLM Nemotron 3 cookbook).
- Available VRAM on the Brev instance before committing to Super vs Nano.
