...
*Lane Agent (Person 4):* the doorway (`airtight/call_model`, the only legal model hop — HiddenLayer's slot is `_analyze`, reasoning toggle in `_reasoning_params`) + four shapes, the M1 loop, **M2** guardrails (all 5 hops), **M3** memory + episodic compounding, **M4** ablation harness, **M5/M6** containment sim (`containment/`). *Surface:* two frames — intake `/` and engine `/admin` (`surface/`, static HTML/CSS/JS, no build step; read-side logic in `sources.py`/`explain.py`, drafting in `jobs.py`). *Data:* live USPTO ODP puller (`data/pull_uspto.py`). Verify: `.venv/bin/pytest tests/` — **expect 149 passed** (0 skipped) with `.[dev,web,poison]`, stub mode, no network — `data/real/` now ships in the repo, so `test_real_pull_splits_cleanly` runs by default and only skips if you delete it. Drop an extra and its tests `importorskip`: `.[dev,web]` drops the two poison-PDF tests (need `pdfplumber`) → 147 passed, 2 skipped; `.[dev]` alone drops all 20 surface tests too → 127 passed, 22 skipped. A green run is not by itself proof coverage — check the count.

**Current focus — `docs/WORKSTREAMS.md`. Blocks B, C and D are done (2026-07-18); A remains.** (A) make OpenShell/NemoClaw actually enforce instead of `print()`. (B) the loop now self-corrects (revise turn in `agent/loop.py`) and compounds (episodic write in `agent/episodes.py`, isolated from the ablation by the harness passing no sink) — the mechanism is proven offline, the live quality gain is unmeasured. B was authored in a separate tree; merging it at `d899e26` fired the predicted collision with C's trust gate (self-generated lessons minted at confidence 1.0 bought reserved statute slots), closed by `201e8f8`. (C) retrieval is now statute-diversified (`d1c60b1`) and BM25-ranked, with a write API — **read C2's caveat before quoting it: what improved is disclosure-specificity (cross-disclosure Jaccard 0.172 → 0.098), not length bias, and the deviation from the literature-default `b` is deliberate and measured.** (D) ingest now distils admitted text into `memory/ingested/` behind a quarantine gate that sits *upstream of the model*. A3 is unblocked; only the sandbox items wait on hosted hardware. **The single highest-leverage item is still the GPU re-run** — retrieval has now changed twice, so neither live ablation number is quotable until it lands, and `agent/memory.py` should be frozen at a recorded SHA when the window opens.
