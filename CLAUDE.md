...
*Lane Agent (Person 4):* the doorway (`airtight/call_model`, the only legal model hop — HiddenLayer's slot is `_analyze`, reasoning toggle in `_reasoning_params`) + four shapes, the M1 loop, **M2** guardrails (all 5 hops), **M3** memory + episodic compounding, **M4** ablation harness, **M5/M6** containment sim (`containment/`). *Surface:* two frames — intake `/` and engine `/admin` (`surface/`, static HTML/CSS/JS, no build step; read-side logic in `sources.py`/`explain.py`, drafting in `jobs.py`). *Data:* live USPTO ODP puller (`data/pull_uspto.py`). Verify: `.venv/bin/pytest tests/` — **expect 171 passed** (0 skipped) with `.[dev,web,poison]`, stub mode, no network — `data/real/` now ships in the repo, so `test_real_pull_splits_cleanly` runs by default and only skips if you delete it. Drop an extra and its tests `importorskip`: `.[dev,web]` drops the two poison-PDF tests (need `pdfplumber`) → 169 passed, 2 skipped; `.[dev]` alone drops all 29 surface tests too → 140 passed, 31 skipped. A green run is not by itself proof coverage — check the count.
