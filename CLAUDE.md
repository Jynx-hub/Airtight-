...
*Lane Agent (Person 4):* the doorway (`airtight/call_model`, the only legal model hop — HiddenLayer's slot is `_analyze`, reasoning toggle in `_reasoning_params`) + four shapes, the M1 loop, **M2** guardrails (all 5 hops), **M3** memory + episodic compounding, **M4** ablation harness, **M5/M6** containment sim (`containment/`). *Surface:* FastAPI starter (`surface/`). *Data:* live USPTO ODP puller (`data/pull_uspto.py`). Verify: `.venv/bin/pytest tests/` — **expect 111 passed** (0 skipped) with `.[dev,web,poison]`, stub mode, no network — `data/real/` now ships in the repo, so `test_real_pull_splits_cleanly` runs by default and only skips if you delete it. Drop an extra and its tests `importorskip`: `.[dev,web]` drops the two poison-PDF tests (need `pdfplumber`) → 109 passed, 2 skipped; `.[dev]` alone drops the surface tests too → 105 passed, 6 skipped. A green run is not by itself proof coverage — check the count.

**Current focus — `docs/WORKSTREAMS.md`. Blocks C and D are done (2026-07-18); A and B remain.** (A) make OpenShell/NemoClaw actually enforce instead of `print()`; (B) make the loop recursive — it still has no revise turn and no episode has ever been written. (C) retrieval is now statute-diversified (`d1c60b1`) and BM25-ranked, with a write API — **read C2's caveat before quoting it: what improved is disclosure-specificity (cross-disclosure Jaccard 0.172 → 0.098), not length bias, and the deviation from the literature-default `b` is deliberate and measured.** (D) ingest now distils admitted text into `memory/ingested/` behind a quarantine gate that sits *upstream of the model*. A3 and B are unblocked; only the sandbox items wait on hosted hardware. **The single highest-leverage item is still the GPU re-run** — retrieval has now changed twice, so neither live ablation number is quotable until it lands, and `agent/memory.py` should be frozen at a recorded SHA when the window opens.
